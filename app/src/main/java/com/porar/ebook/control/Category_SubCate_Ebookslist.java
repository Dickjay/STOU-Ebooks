...
public class Category_SubCate_Ebookslist {
	...
	public Category_SubCate_Ebookslist(Context context, int CatID, int SCatID) {
	...
	public void LoadCategoryAPI() {
		...
		asyncTask_FetchAPI.setOnFetchListener(new OnFetchAPIListener() {
			...
			//	Log.e("LoadCategoryAPI", "" + apiURL);
				AppMain.pList_Subcategories_ebook = result;
				handler.post(new Runnable() {
		...
		asyncTask_FetchAPI.execute(
				AppMain.getAPIbyRefKey("categories-list", "scatid=" + SCatID + "&page=0" + "&cid=" + Shared_Object.getCustomerDetail.getCID() )
		);
