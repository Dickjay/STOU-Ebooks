...
public abstract class View_SearchPublisher {
	...
	Adapter_SearchList_Publisher adapter_SearchList;
	Ebook_Detail ebook_Detail;
	private int id_head = 0;

	public View_SearchPublisher(Activity context, LinearLayout linear_fotTypeEbook, RelativeLayout rt_dummyscreen, int id_head) {
	...
	public void setAdapter(Adapter_SearchList_Publisher adapter_SearchList) {
	...
	private void performClick(String text) {
		...
		publishersList = new ArrayList<Model_Publisher>();
		...
		asyncTask_FetchAPI.setOnFetchListener(new OnFetchAPIListener() {
			...
			public void onFetchComplete(String apiURL, int currentIndex, PList result) {
				...
				for (PListObject each : (Array) result.getRootElement()) {
					publishersList.add(new Model_Publisher(each));
				}

				handler.post(new Runnable() {
					@Override
					public void run() {
						adapter_SearchList = new Adapter_SearchList_Publisher(context, 0, publishersList);
						lv_search.setAdapter(adapter_SearchList);
	...
	public abstract void onAdapter_SearchList(Adapter_SearchList_Publisher adapter_SearchList);
}
