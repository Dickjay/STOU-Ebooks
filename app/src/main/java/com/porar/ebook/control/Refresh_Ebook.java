...
public class Refresh_Ebook {
	...
	public Refresh_Ebook(Context context) {
	...
	//			Log.e("onFetchStart", apiURL);
		...
		//		Log.e("refresh_api", "" + apiURL);
				AppMain.pList_default_ebook = result;
		...
		if (Fragment_Ebook.lastUrl != null) {
			if (Fragment_Ebook.lastUrl.getUrl() != null) {
				String new_url = Fragment_Ebook.lastUrl.getUrl();
				if (new_url.toString().length() > 0) {
					asyncTask_FetchAPI.execute(Fragment_Ebook.lastUrl.getUrl());
					tag_category = true;
				}
			}
			if (AppMain.pList_default_ebook != null) {
				if (!tag_category) {
					asyncTask_FetchAPI.execute(AppMain.getAPIbyRefKey("ebooks-list", "type=N&count=80&retina=0&catid=0"));
				}
			} else {
				if (!tag_category) {
					asyncTask_FetchAPI.execute(AppMain.getAPIbyRefKey("ebooks-list", "type=N&count=80&retina=0&catid=0"));
