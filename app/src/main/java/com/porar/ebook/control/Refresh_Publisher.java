...
public class Refresh_Publisher {
	...
	public Refresh_Publisher(Context context) {
	...
	//			Log.e("refresh_api", "" + apiURL);
		...
		if (Fragment_Publisher.lastUrl != null) {
			if (Fragment_Publisher.lastUrl.getUrl() != null) {
				String new_url = Fragment_Publisher.lastUrl.getUrl();
				if (new_url.toString().length() > 0) {
					asyncTask_FetchAPI.execute(Fragment_Publisher.lastUrl.getUrl());
					tag_category = true;
				}
			}
			if (AppMain.pList_default_ebook != null) {
				if (!tag_category) {
					asyncTask_FetchAPI.execute(AppMain.getAPIbyRefKey("publishers-list", "type=U&short=&page=&count=100"));
				}
			}else{
				if (!tag_category) {
					asyncTask_FetchAPI.execute(AppMain.getAPIbyRefKey("publishers-list", "type=U&short=&page=&count=100"));
