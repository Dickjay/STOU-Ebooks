...
public class Category_SubCate {
	...
	public Category_SubCate(Context context, int CatID) {
	...
	public void LoadCategoryAPI() {
		...
		asyncTask_FetchAPI.setOnFetchListener(new OnFetchAPIListener() {
			...
			//	Log.e("LoadCategoryAPI", "" + apiURL);
				AppMain.pList_Subcategories = result;
				handler.post(new Runnable() {
		...
		asyncTask_FetchAPI.execute(
				AppMain.getAPIbyRefKey("categories", "catid=" + CatID));
