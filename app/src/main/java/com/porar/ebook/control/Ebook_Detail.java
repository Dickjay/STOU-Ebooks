...
public class Ebook_Detail {
	...
	public Ebook_Detail(Context context, String bid) {
	...
	//			Log.e("onFetchError", e.toString());
		...
		//		Log.w("LoadEbooksDetailAPI", "" + apiURL);
				...
				if (apiURL.contains("ebooks-detail")) {
					plist_detail = result;
				}
			}
		...
		asyncTask_FetchAPI.execute(AppMain.getAPIbyRefKey("ebooks-detail", "chcode=AU&bid=" + bid + "&cid=" + cid + "&did=" + did + "&version=1.5"));
//and quote
	}
